...
lsmeans(cond_site, pairwise ~ Site) # no signif, 8A higher
...
Since there are differences in condition between sites, and condition is measured across spiders, let's also measure condition relative to each site. 
...
choice_spider %>% 
  ...
  xlab("Day") +
  ylab("Proportion of Spiders that Stayed Four Days") +
  scale_fill_manual("Chamber Part",
...
Just by observation, it looks like spiders from urban sites are more likely to choose quiet when younger and then loud when older. Only two spiders were found in the tunnel all four days.

## Stayed all four days

**tl;dr:** When we used a binomial regression to assess the probability of staying all four days (1) and moving (0), there was a significant interaction between leq and condition where, at lower leq, spiders were more likely to move if in below-average condition. There was a trend in the interaction between side and age where spiders became less likely to stay in loud with age, but no changes of staying in quiet. 
...
# scale all the related continuous variables
stay_scaled <- stay %>% 
  ungroup() %>% 
...
Let's see what predicts whether a spider stays in the same part all four days. We will remove observations of spiders in the tunnel to simplify analyses since only two spiders stayed in the tunnel all four days (or more than two days). This only removed two that stayed and none that moved. We will use a binomial regression since we can code the data as stayed all four days (1) or moved at least once (0). Since we know age and condition are positively correlated, we will avoid using interactions between age and condition and we will double check the VIF of the final model after backward selection.
...
kable(Anova(stay.binom)) # trend mean_leq:age
...
kable(Anova(stay.binom2)) #trend condition
...
Mean Leq (Chi = `r round(Anova(stay.binom)[1, 1], 3)`, df = `r stay.binom$df.residual`, P `r ifelse(round(Anova(stay.binom)[1,3], 3) == 0, print("< 0.001"), print(paste("=", round(Anova(stay.binom)[1, 3], 3))))`) and age (Chi = `r round(Anova(stay.binom)[2, 1], 3)`, df = `r stay.binom$df.residual`, P `r ifelse(round(Anova(stay.binom)[2,3], 3) == 0, print("< 0.001"), print(paste("=", round(Anova(stay.binom)[2, 3], 3))))`) did not significantly affect the probability of staying or moving. 

There was a trend or an interaction between mean Leq and age (Chi = `r round(Anova(stay.binom)[3, 1], 3)`, df = `r stay.binom$df.residual`, P `r ifelse(round(Anova(stay.binom)[3,3], 3) == 0, print("< 0.001"), print(paste("=", round(Anova(stay.binom)[3, 3], 3))))`).

For those that stayed, there was a trend for condition (Chi = `r round(Anova(stay.binom2)[1, 1], 3)`, df = `r stay.binom$df.residual`, P `r ifelse(round(Anova(stay.binom2)[1,3], 3) == 0, print("< 0.001"), print(paste("=", round(Anova(stay.binom2)[1, 3], 3))))`). 

```{r stay graph, echo = FALSE, message = FALSE, warning = FALSE, results = 'hide'}
#Leq by Age
pred <- data.frame(effect("mean_leq*Age", quantiles = c(0.25, 0.75), stay.binom, residuals = TRUE))
# back transform
...
pred <- pred %>% 
  mutate(Age = factor(round(Age)),
         Age = fct_recode(Age, "younger" = "17", "older" = "32"))

#raw data
data <- stay2 %>% 
  mutate(Age = ifelse(Age < median(choice$Age), "younger", "older"),
         Age = factor(Age),
         Age = fct_relevel(Age, "younger", "older"))

ggplot() +
  geom_jitter(aes(x = mean_leq, y = stay, color = Age), data = data, width = 1, height = 0.05, alpha = 0.5) +
  geom_hline(yintercept = 0.5, linetype = 2) +
  geom_line(aes(x = mean_leq, y = fit, color = Age), data = pred) +
  geom_ribbon(aes(x = mean_leq, y = fit, ymin = fit - (1 * se), ymax = fit + (1 * se), color = Age, fill = Age), data = pred, alpha = 0.5) +
  xlab("Site Average Leq (dB, 20-1000 Hz)") +
  ylab("Proportion of Spiders that Stayed Four Days") +
  scale_x_continuous(breaks = c(-69, -64, -55)) +
  scale_color_manual("Age", 
                     values = c("#e7298a", "#666666"),
...
So the only thing that predicts staying all four days is an interaction between age and mean_leq. Younger spiders are increasing likely to stay as leq increases. 
...
Anova(stay.binom.5a2, test.statistic = "Wald") # none
```

Age mattered for spiders from the loudest sites, but not the rest (although it was retained for the second loudest site).

## Spiders in the Tunnel 

**tl;dr:** Nothing significantly predicted when a spider was found in the tunnel except a trend in age by leq where younger spiders showed decreasing likelihood to be in the tunnel with increasing leq. 
...
```{r tunnel stats and assumptions, echo = FALSE, message = FALSE, warning = FALSE}
...
#tunnel.mod <- glmer(tunnel ~ mean_leq * day * Age + mean_leq * day * condition #+ (1|ID), data = tunnel_scaled, family = binomial)
...
#tunnel.mod6 <- update(tunnel.mod5, .~. -mean_leq:day:Age)
#drop1(tunnel.mod6, test = "Chisq")
#tunnel.mod7 <- update(tunnel.mod6, .~. -mean_leq:day)
#drop1(tunnel.mod7, test = "Chisq")
#tunnel.mod8 <- update(tunnel.mod7, .~. -day:Age)
#drop1(tunnel.mod8, test = "Chisq")
#tunnel.mod9 <- update(tunnel.mod8, .~. -day)
#drop1(tunnel.mod9, test = "Chisq")
#Anova(tunnel.mod9)
# mean_leq x age trend

tunnel.mod <- glmer(tunnel ~ mean_leq * Age + (1|ID), data = tunnel_scaled, family = binomial)
Anova(tunnel.mod, test.statistic = "Chisq")
...
```{r tunnel stats and assumptions for sites, echo = FALSE, message = FALSE, warning = FALSE}
tunnel_8B <- choice_spider_8B %>% 
  mutate(tunnel = ifelse(side_lq == "tunnel", 1, 0))
tunnel_8B_scaled <- tunnel_8B %>% 
  mutate(Age = c(scale(Age)),
         condition = c(scale(condition)),
         mean_leq = c(scale(mean_leq)))

tunnel.mod.8b <- glmer(tunnel ~ day * Age + day * condition + (1|ID), data = tunnel_8B_scaled, family = binomial)
drop1(tunnel.mod.8b, test = "Chisq")
tunnel.mod.8b2 <- update(tunnel.mod.8b, .~. -day:condition)
drop1(tunnel.mod.8b2, test = "Chisq")
tunnel.mod.8b3 <- update(tunnel.mod.8b2, .~. -condition)
drop1(tunnel.mod.8b3, test = "Chisq")
Anova(tunnel.mod.8b3) # day x age no diff

tunnel_8A <- choice_spider_8A %>% 
  mutate(tunnel = ifelse(side_lq == "tunnel", 1, 0))
tunnel_8A_scaled <- tunnel_8A %>% 
  mutate(Age = c(scale(Age)),
         condition = c(scale(condition)),
         mean_leq = c(scale(mean_leq)))

tunnel.mod.8a <- glmer(tunnel ~ day * Age + day * condition + (1|ID), data = tunnel_8A_scaled, family = binomial)
drop1(tunnel.mod.8a, test = "Chisq")
tunnel.mod.8a2 <- update(tunnel.mod.8a, .~. -day:condition)
drop1(tunnel.mod.8a2, test = "Chisq")
tunnel.mod.8a3 <- update(tunnel.mod.8a2, .~. -condition)
drop1(tunnel.mod.8a3, test = "Chisq")
tunnel.mod.8a4 <- update(tunnel.mod.8a3, .~. -day:Age)
drop1(tunnel.mod.8a4, test = "Chisq")
tunnel.mod.8a5 <- update(tunnel.mod.8a4, .~. -day)
drop1(tunnel.mod.8a5, test = "Chisq")
Anova(tunnel.mod.8a5) # Age no diff

tunnel_6C <- choice_spider_6C %>% 
  mutate(tunnel = ifelse(side_lq == "tunnel", 1, 0))
tunnel_6C_scaled <- tunnel_6C %>% 
  mutate(Age = c(scale(Age)),
         condition = c(scale(condition)),
         mean_leq = c(scale(mean_leq)))

tunnel.mod.6c <- glmer(tunnel ~ day * Age + day * condition + (1|ID), data = tunnel_6C_scaled, family = binomial)
drop1(tunnel.mod.6c, test = "Chisq")
tunnel.mod.6c2 <- update(tunnel.mod.6c, .~. -day:condition)
drop1(tunnel.mod.6c2, test = "Chisq")
tunnel.mod.6c3 <- update(tunnel.mod.6c2, .~. -condition)
drop1(tunnel.mod.6c3, test = "Chisq")
Anova(tunnel.mod.6c3) # day by age
vif(tunnel.mod.6c3) # suspicious all 1

tunnel_5A <- choice_spider_5A %>% 
  mutate(tunnel = ifelse(side_lq == "tunnel", 1, 0))
tunnel_5A_scaled <- tunnel_5A %>% 
  mutate(Age = c(scale(Age)),
         condition = c(scale(condition)),
         mean_leq = c(scale(mean_leq)))

tunnel.mod.5a <- glmer(tunnel ~ day * Age + day * condition + (1|ID), data = tunnel_5A_scaled, family = binomial)
drop1(tunnel.mod.5a, test = "Chisq")
tunnel.mod.5a2 <- update(tunnel.mod.5a, .~. -day:Age)
drop1(tunnel.mod.5a2, test = "Chisq")
tunnel.mod.5a3 <- update(tunnel.mod.5a2, .~. -day:condition)
drop1(tunnel.mod.5a3, test = "Chisq")
tunnel.mod.5a4 <- update(tunnel.mod.5a3, .~. -day)
drop1(tunnel.mod.5a4, test = "Chisq")
tunnel.mod.5a5 <- update(tunnel.mod.5a4, .~. -condition)
drop1(tunnel.mod.5a5, test = "Chisq")
Anova(tunnel.mod.5a5) # age
```

Mean Leq (Chi = `r round(Anova(tunnel.mod)[1, 1], 3)`, df = `r summary(tunnel.mod)$devcomp$dims[4]`, P `r ifelse(round(Anova(tunnel.mod)[1,3], 3) == 0, print("< 0.001"), print(paste("=", round(Anova(tunnel.mod)[1, 3], 3))))`) and age (Chi = `r round(Anova(tunnel.mod)[2, 1], 3)`, df = `r summary(tunnel.mod)$devcomp$dims[4]`, P `r ifelse(round(Anova(tunnel.mod)[2,3], 3) == 0, print("< 0.001"), print(paste("=", round(Anova(tunnel.mod)[2, 3], 3))))`) did not significantly affect the probability of staying or moving.

There was a trend of an interaction between mean Leq and age (Chi = `r round(Anova(tunnel.mod)[3, 1], 3)`, df = `r summary(tunnel.mod)$devcomp$dims[4]`, P `r ifelse(round(Anova(tunnel.mod)[3,3], 3) == 0, print("< 0.001"), print(paste("=", round(Anova(tunnel.mod)[3, 3], 3))))`). The probability of being found in the tunnel decreased with leq for young spiders.

```{r tunnel graphs for leq, echo = FALSE, message = FALSE, warning = FALSE, results = 'hide'}
#raw data
data <- tunnel %>% 
  mutate(Age = ifelse(Age < median(Age), "younger", "older"),
         Age = factor(Age),
         Age = fct_relevel(Age, "younger", "older"))

pred <- data.frame(effect("mean_leq*Age", tunnel.mod, residuals = TRUE, quantiles = c(0.25, 0.75)))
...
pred <- pred %>% 
  mutate(Age = factor(Age),
         Age = fct_recode(Age, "younger" = "15", "older" = "32"),
         Age = fct_relevel(Age, "younger", "older"))

ggplot() +
  geom_jitter(aes(x = mean_leq, y = tunnel, color = Age), data = data, width = 1, height = 0.05, alpha = 0.5) +
  geom_line(aes(x = mean_leq, y = fit, color = Age), data = pred) +
  geom_ribbon(aes(x = mean_leq, y = fit, ymin = fit - (1 * se), ymax = fit + (1 * se), color = Age, fill = Age), data = pred, alpha = 0.5) +
  xlab("Site Average Leq (dB, 20-1000 Hz)") +
  ylab("Proportion of Spiders that were Found in the Tunnel") +
  scale_x_continuous(breaks = c(-69, -64, -55)) +
  scale_color_manual("Age", 
                     values = c("#e7298a", "#666666"),
                     labels = c("Younger", "Older")) +
...
```{r loud quiet stats and assumptions, echo = FALSE, message = FALSE, warning = FALSE}
...
lq1 <- glmer(side ~ mean_leq * day * Age + mean_leq * day * condition + (1|ID), data = choice_lq, family = binomial)
...
lq <- glm(side ~ mean_leq * Age, data = choice_lq, family = binomial)
Anova(lq, test.statistic = "Wald")

pred = expand.grid(mean_leq = seq(-1, 1.4, 0.02),
                   #mean_leq = c(min(stay_scaled$mean_leq),
                  #              median(stay_scaled$mean_leq),
                  #              max(stay_scaled$mean_leq)),
                 Age = quantile(choice_lq$Age, probs = c(0.25, 0.5, 0.75)))
pred <- add_column(pred, fit = data.frame(predict(lq, newdata = pred, type = "response", se.fit = TRUE))$fit,
                  se = data.frame(predict(lq, newdata = pred, type = "response", se.fit = TRUE))$se.fit )
# back transform
pred$Age = pred$Age * sd(choice_spider$Age) + mean(choice_spider$Age)
pred$mean_leq = pred$mean_leq * sd(choice_spider$mean_leq) + mean(choice_spider$mean_leq)

ggplot() +
  geom_line(aes(x = mean_leq, y = fit, color = factor(Age)), data = pred) +
  geom_ribbon(aes(x = mean_leq, y = fit, ymin = fit - se, ymax = fit + se, fill = factor(Age)), data = pred, alpha = 0.5) +
  geom_hline(yintercept = 0.5, linetype = 2)
```
...
choice %>% 
  ...
  group_by(Site_cat, mean_leq, Origin) %>% 
...
#drop1(binom_leq, test = "Chisq")
binom_leq2 <- update(binom_leq, .~. -mean_leq:condition)
#drop1(binom_leq2, test = "Chisq")
binom_leq3 <- update(binom_leq2, .~. -condition)
#drop1(binom_leq3, test = "Chisq")
kable(coef(summary(binom_leq3)), digits = 3)
#simulateResiduals(binom_leq3, plot = TRUE) # dispersion test
```

There is no overall effect of site average Leq on the likelihood of choosing loud or quiet (z = `r round(coef(summary(binom_leq3))[2, 3], 2)`, df = `r binom_leq3$df.resid`, P `r ifelse(round(coef(summary(binom_leq3))[2,4], 3) == 0, print("< 0.001"), print(paste("=", round(coef(summary(binom_leq3))[2,4], 3))))`). 

There is no overall effect of age on the likelihood of choosing loud or quiet (z = `r round(coef(summary(binom_leq3))[3, 3], 2)`, df = `r binom_leq3$df.resid`, P `r ifelse(round(coef(summary(binom_leq3))[3,4], 3) == 0, print("< 0.001"), print(paste("=", round(coef(summary(binom_leq3))[3,4], 3))))`). 

There is a trend of an interaction between site leq and age; specifically, younger spiders showed increasing preference for loud vibratory environments as origin leq increased, but older spiders show no preference (z = `r round(coef(summary(binom_leq3))[4, 3], 2)`, df = `r binom_leq3$df.resid`, P `r ifelse(round(coef(summary(binom_leq3))[4,4], 3) == 0, print("< 0.001"), print(paste("=", round(coef(summary(binom_leq3))[4,4], 3))))`).
...
choice_8B <- choice %>% 
  filter(Site_cat == "8B")
binom_8B <- glm(side ~ 1, data = choice_8B, family = binomial(link = 'logit'))
binom_8B2 <- glm(side ~ Age, data = choice_8B, family = binomial(link = 'logit'))
#anova(binom_8B, binom_8B2, test = "Chi") # let's keep age
coef_binom_8B <- round(data.frame(coef(summary(binom_8B2))), 3) %>% 
  mutate(Site = "8B",
         Leq = -55)
  
choice_8A <- choice %>% 
  filter(Site_cat == "8A")
binom_8A <- glm(side ~ 1, data = choice_8A, family = binomial(link = 'logit'))
binom_8A2 <- glm(side ~ Age, data = choice_8A, family = binomial(link = 'logit'))
#anova(binom_8A, binom_8A, test = "Chi") # without age best
...
choice_6C <- choice %>% 
  filter(Site_cat == "6C")
binom_6C <- glm(side ~ 1, data = choice_6C, family = binomial(link = 'logit'))
binom_6C2 <- glm(side ~ Age, data = choice_6C, family = binomial(link = 'logit'))
#anova(binom_6C, binom_6C2, test = "Chi") # without age better
...
choice_5A <- choice %>% 
  filter(Site_cat == "5A")
binom_5A <- glm(side ~ 1, data = choice_5A, family = binomial(link = 'logit'))
binom_5A2 <- glm(side ~ Age, data = choice_5A, family = binomial(link = 'logit'))
#anova(binom_5A, binom_5A2, test = "Chi") # let's keep age
coef_binom_5A <- round(data.frame(coef(summary(binom_5A2))), 3) %>% 
  mutate(Site = "5A",
         Leq = -69)
...
pred_leq <- data.frame(Effect("mean_leq", binom_leq3, residuals = TRUE))
# back transform
pred_leq$mean_leq = pred_leq$mean_leq * sd(choice$mean_leq) + mean(choice$mean_leq)
...
leq <- ggplot() +
  ...
  geom_ribbon(aes(x = mean_leq, ymin = fit - (se * 1.96), ymax = fit + (se * 1.96)), data = pred_leq, alpha = 0.5) +
  scale_x_continuous(breaks = c(-69, -64, -55)) +
...
pred_age <- data.frame(Effect("Age", binom_leq3, residuals = TRUE))
# back transform
pred_age$Age = pred_age$Age * sd(choice$Age) + mean(choice$Age)

age <- ggplot() +
  geom_bar(aes(fill=side_w_more, y = n, x = quant_Age), position="fill", stat="identity", data = stacked, alpha = 0.75) +
  geom_hline(yintercept = 0.5, linetype = 2) +
...
ggarrange(leq, age, labels = c("A", "B"), common.legend = TRUE)
...
nd <- add_column(nd, fit = predict(binom_leq3, newdata = nd, type = "response"),
                 se = predict(binom_leq3, newdata = nd, se.fit = TRUE, type = "response")$se.fit)
# back transform
...
nd = expand.grid(mean_leq = c(-0.8889257, -0.1045795, 1.3072437),
                Age = seq(-2.5, 2.5, 0.2))
nd <- add_column(nd, fit = predict(binom_leq3, newdata = nd, type = "response"),
                 se = predict(binom_leq3, newdata = nd, type = "response", se.fit = TRUE)$se.fit)
# back transform
nd$Age = nd$Age * sd(choice$Age) + mean(choice$Age)
...
ggplot(aes(x = Age, y = fit, group = factor(mean_leq), color = factor(mean_leq), fill = factor(mean_leq))) +
  ...
  scale_y_continuous(limits = c(-0.25, 1.1), breaks = c(-0.25, 0, 0.25, 0.5, 0.75, 1)) +
...
# origin for funsies
nd = expand.grid(Origin = levels(factor(choice$Origin)),
                Age = seq(-2.5, 2.5, 0.2))
nd <- add_column(nd, fit = predict(binom_org3, newdata = nd, type = "response"),
                 se = predict(binom_org3, newdata = nd, type = "response", se.fit = TRUE)$se.fit)
# back transform
nd$Age = nd$Age * sd(choice$Age) + mean(choice$Age)
...
ggplot(aes(x = Age, y = fit, group = Origin, color = Origin, fill = Origin)) +
  ...
  ylab("Proportion of Spiders") +
  xlab("Age (days since maturation)") +
  scale_y_continuous(limits = c(-0.2, 1.1), breaks = c(0, 0.25, 0.5, 0.75, 1)) +
...
binom_noage <- ggplot() +
  geom_bar(aes(fill = side_w_more, y = n, x = Site_cat), position="fill", stat="identity", data = stacked_noage, alpha = 0.75) +
  geom_hline(yintercept = 0.5, linetype = 2) +
  ...
  geom_text(aes(x = Site_cat, y = 1.05, label = paste("n = ", n)), data = nval_noage, size = 3) +
...
beta_leq <- betareg(propl ~ mean_leq * Age, data = choice_scaled)
...
There is no overall effect of site average Leq on the proportion of silk on the loud or quiet side (z = `r round(summary(beta_leq_gam)$p.t[2], 2)`, df = `r summary(beta_leq_gam)$residual.df`, P `r ifelse(round(summary(beta_leq_gam)$p.pv[2], 3) == 0, print("< 0.001"), print(paste("=", round(summary(beta_leq_gam)$p.pv[2], 3))))`). 

There is no overall effect of age on the proportion of silk on the loud or quiet side (z = `r round(summary(beta_leq_gam)$p.t[3], 2)`, df = `r summary(beta_leq_gam)$residual.df`, P `r ifelse(round(summary(beta_leq_gam)$p.pv[3], 3) == 0, print("< 0.001"), print(paste("=", round(summary(beta_leq_gam)$p.pv[3], 3))))`). 

There is a significant interaction between site leq and age; specifically, younger spiders showed increasing choice for loud vibratory environments as origin leq increased, but older spiders show no preference (z = `r round(summary(beta_leq_gam)$p.t[4], 2)`, df = `r summary(beta_leq_gam)$residual.df`, P `r ifelse(round(summary(beta_leq_gam)$p.pv[4], 3) == 0, print("< 0.001"), print(paste("=", round(summary(beta_leq_gam)$p.pv[4], 3))))`).
...
rbind(coef_beta_8B_age, coef_beta_8B, coef_beta_8A, coef_beta_6C, coef_beta_5A)
```

Again, only 8B significantly differs from 50% and differs by age. 

```{r origin test for beta, include = FALSE, warning = FALSE, message = FALSE}
...
beta_leq_u <- gam(propl ~ Age, data = choice_urban, family = betar(link = "logit"))
summary(beta_leq_u)

beta_leq_r <- gam(propl ~ 1, data = choice_rural, family = betar(link = "logit"))
summary(beta_leq_r)
```

```{r looking at tunnel separate test, include = FALSE, warning = FALSE, message = FALSE}
...
We get similar results when we include tunnel in the total. When we look at the proportion of silk in the tunnel, there is a significant increase by leq (z = `r round(summary(beta_leq_tunnel)$p.t[2], 2)`, df = `r summary(beta_leq_tunnel)$residual.df`, P `r ifelse(round(summary(beta_leq_tunnel)$p.pv[2], 3) == 0, print("< 0.001"), print(paste("=", round(summary(beta_leq_tunnel)$p.pv[2], 3))))`).
...
beta_leqt <- gam(prop_part ~ part * mean_leq * Age, data = choice_part_scaled, family = betar(link = "logit"))
anova(beta_leqt)
...
prop_site <- ggplot() +
  geom_rect(aes(xmin = -Inf, xmax = Inf, ymin = 0.5, ymax = 1), fill = "#D95F02", alpha = 0.5) +
  geom_rect(aes(xmin = -Inf, xmax = Inf, ymin = 0, ymax = 0.5), fill = "#1B9E77", alpha = 0.5) +
  geom_point(aes(x = Site_cat, y = propl), data = choice, alpha = 0.75, color = "grey20") +
  geom_hline(yintercept = 0.5, linetype = 2) +
  ...
  geom_text(aes(x = Site_cat, y = 1.05, label = paste("n = ", n)), data = nval, size = 3) +
scale_x_discrete(labels = c("5A" = "5A \n(-69 dB)", "6C" = "6C \n(-69 dB)", "8A" = "8A \n(-64 dB)", "8B" = "8B \n(-55 dB)")) +
  scale_fill_manual("Side with \nMore Silk", 
                    values = c("#1B9E77", "#D95F02"),
                    labels = c("Quiet", "Loud")) +
...
pred <- expand.grid(mean_leq = seq(-1, 1.4, 0.1),
                    Age = quantile(choice_part_scaled$Age, probs = c(0.25, 0.75)),
...
**tl;dr:** We used a negative binomial regression to test for differences in total silk use. Again, body condition had no effect. Total silk mass marginally decreased with site average Leq and significantly increased with age (especially for 8B and 6C). When we include silk mass in the tunnel in the total, leq is dropped from the model. 
...
kable(round(coef(summary(total4)), 3))
```

Only age and origin leq remained in the model.

There is a trend that silk mass decreased with increasing origin leq (z = `r round(coef(summary(total4))[2, 3], 2)`, df = `r total4$df.residual`, P `r ifelse(round(coef(summary(total4))[2, 4], 3) == 0, print("< 0.001"), print(paste("=", round(coef(summary(total4))[2, 4], 3))))`). 

The amount of silk increased with increasing age (z = `r round(coef(summary(total4))[3, 3], 2)`, df = `r total4$df.residual`, P `r ifelse(round(coef(summary(total4))[3, 4], 3) == 0, print("< 0.001"), print(paste("=", round(coef(summary(total4))[3, 4], 3))))`). 
...
kable(round(coef(summary(total_tunnel5)), 3))
```

Only age remained in the model.

The amount of silk increased with increasing age (z = `r round(coef(summary(total_tunnel5))[2, 3], 2)`, df = `r total_tunnel5$df.residual`, P `r ifelse(round(coef(summary(total_tunnel5))[2, 4], 3) == 0, print("< 0.001"), print(paste("=", round(coef(summary(total_tunnel5))[2, 4], 3))))`). 

Let's look at each site. 

```{r total silk of each site, echo = FALSE, warning = FALSE, message = FALSE}
total_8B <- glm.nb(total_micro ~ Age, data = choice_8B)
total_tunnel_8B <- glm.nb(total_micro_tunnel ~ Age, data = choice_8B) #same
#drop1(total_8B, test = "Chisq") # keep age
coef_8B <- data.frame(round(coef(summary(total_8B)), 3),
                      Site = "8B",
                      mean_leq = -55)

total_8A <- glm.nb(total_micro ~ Age, data = choice_8A)
total_tunnel_8A <- glm.nb(total_micro_tunnel ~ Age, data = choice_8A) #same
#drop1(total_8A, test = "Chisq") #drop age
coef_8A <- data.frame(round(coef(summary(total_8A)), 3),
                      Site = "8A",
                      mean_leq = -64)

total_6C <- glm.nb(total_micro ~ Age, data = choice_6C)
total_tunnel_6C <- glm.nb(total_micro_tunnel ~ Age, data = choice_6C) #same
#drop1(total_6C, test = "Chisq") # keep age
coef_6C <- data.frame(round(coef(summary(total_6C)), 3),
                      Site = "6C",
                      mean_leq = -69)

total_5A <- glm.nb(total_micro ~ Age, data = choice_5A)
total_tunnel_5A <- glm.nb(total_micro_tunnel ~ Age, data = choice_5A) #same
#drop1(total_5A, test = "Chisq") #drop age
coef_5A <- data.frame(round(coef(summary(total_5A)), 3),
                      Site = "5A",
                      mean_leq = -69)
...
test_silk <- augment(total4, data = choice_scaled)
...
pred = expand.grid(mean_leq = seq(-1, 1.4, 0.02),
                 Age = quantile(choice_scaled$Age, probs = c(0.25, 0.75)))
pred <- add_column(pred, fit = data.frame(predict(total4, newdata = pred, type = "response", se.fit = TRUE))$fit,
                  se = data.frame(predict(total4, newdata = pred, type = "response", se.fit = TRUE))$se.fit )
# back transform
pred$mean_leq = pred$mean_leq * sd(choice$mean_leq) + mean(choice$mean_leq)
pred$Age = pred$Age * sd(choice$Age) + mean(choice$Age)
...
total_silk <- ggplot() +
  geom_point(aes(x = mean_leq, y = total_micro), data = choice2) +
...
pred_8B = expand.grid(Site = "8B",
                Age = seq(0, 50, 1)) 
pred_8B <- add_column(pred_8B, fit = predict(total_8B, type = "response", newdata = pred_8B),
                      se = predict(total_8B, type = "response", newdata = pred_8B, se.fit = TRUE)$se.fit)

pred_8A = expand.grid(Site = "8A",
                Age = seq(0, 50, 1)) 
pred_8A <- add_column(pred_8A, fit = predict(total_8A, type = "response", newdata = pred_8A),
                      se = predict(total_8A, type = "response", newdata = pred_8A, se.fit = TRUE)$se.fit)

pred_6C = expand.grid(Site = "6C",
                Age = seq(0, 50, 1)) 
pred_6C <- add_column(pred_6C, fit = predict(total_6C, type = "response", newdata = pred_6C),
                      se = predict(total_6C, type = "response", newdata = pred_6C, se.fit = TRUE)$se.fit)

pred_5A = expand.grid(Site = "5A",
                Age = seq(0, 50, 1)) 
pred_5A <- add_column(pred_5A, fit = predict(total_5A, type = "response", newdata = pred_5A),
                      se = predict(total_5A, type = "response", newdata = pred_5A, se.fit = TRUE)$se.fit)
...
choice_side <- choice %>% 
  dplyr::select(ID, Age, condition, mean_leq, Site, Site_cat, Origin, loud_silk, quiet_silk) %>% 
...
The amount of silk on each side did not interact with leq (Chisq = `r round(Anova(silk_side.nb4)[4, 1], 2)`, df = `r silk_side.nb4$df.residual`, P `r ifelse(round(Anova(silk_side.nb4)[4, 3], 3) == 0, print("< 0.001"), print(paste("=", round(Anova(silk_side.nb4)[4, 3], 3))))`) or age (Chisq = `r round(Anova(silk_side.nb4)[5, 1], 2)`, df = `r silk_side.nb4$df.residual`, P `r ifelse(round(Anova(silk_side.nb4)[5, 3], 3) == 0, print("< 0.001"), print(paste("=", round(Anova(silk_side.nb4)[5, 3], 3))))`). 

There is a significant interaction between side, age, and mean_leq (Chisq = `r round(Anova(silk_side.nb4)[6, 1], 2)`, df = `r silk_side.nb4$df.residual`, P `r ifelse(round(Anova(silk_side.nb4)[6, 3], 3) == 0, print("< 0.001"), print(paste("=", round(Anova(silk_side.nb4)[6, 3], 3))))`). 
...
test_silk <- augment(silk_side.nb4, data = choice_side)
resid_silk <- ggplot(test_silk, aes(x = .fitted, y = .resid)) + 
  geom_point() + 
  geom_smooth() +
  geom_hline(yintercept = 0) +
...
pred <- add_column(pred, fit = data.frame(predict(silk_side.nb4, newdata = pred, type = "response", se.fit = TRUE))$fit,
                  se = data.frame(predict(silk_side.nb4, newdata = pred, type = "response", se.fit = TRUE))$se.fit )
# back transform
...
```{r}
choice_side2 <- choice %>% 
  dplyr::select(ID, Age, condition, mean_leq, Site, Site_cat, Origin, loud_silk_v, quiet_silk_v, tunnel_silk_v) %>% 
...
silk_sidet.nb2 <- step(silk_sidet.nb)
Anova(silk_sidet.nb2)
kable(Anova(silk_sidet.nb2))
lsmeans(silk_sidet.nb2, pairwise ~ part)
lstrends(silk_sidet.nb2, pairwise ~ part, var = "mean_leq")
```
...
pred <- add_column(pred, fit = data.frame(predict(silk_sidet.nb2, newdata = pred, type = "response", se.fit = TRUE))$fit,
                  se = data.frame(predict(silk_sidet.nb2, newdata = pred, type = "response", se.fit = TRUE))$se.fit )
# back transform
...
pred_8B = expand.grid(Site = "8B",
                Age = seq(-2.5, 2.5, 0.1),
                part = levels(choice_side2_8B$part)) 
...
pred_8A = expand.grid(Site = "8A",
                Age = seq(-2.5, 2.5, 0.1),
                part = levels(choice_side2_8A$part))
...
pred_6C = expand.grid(Site = "6C",
                Age = seq(-2.5, 2.5, 0.1),
                part = levels(choice_side2_6C$part))
...
pred_5A = expand.grid(Site = "5A",
                Age = seq(-2.5, 2.5, 0.1),
                part = levels(choice_side2_5A$part))
...
There is a trend that the proportional difference decreases with leq (z = `r round(summary(model.beta.gam.diff)$p.t[2], 2)`, df = `r summary(model.beta.gam.diff)$residual.df`, P `r ifelse(round(summary(model.beta.gam.diff)$p.pv[2], 3) == 0, print("< 0.001"), print(paste("=", round(summary(model.beta.gam.diff)$p.pv[2], 3))))`). 

The proportional difference is significantly higher for rural than urban spiders (z = `r round(summary(model.beta.gam.diff2)$p.t[2], 2)`, df = `r summary(model.beta.gam.diff2)$residual.df`, P `r ifelse(round(summary(model.beta.gam.diff2)$p.pv[2], 3) == 0, print("< 0.001"), print(paste("=", round(summary(model.beta.gam.diff2)$p.pv[2], 3))))`). 
...
diff_site <- ggplot() +
  geom_point(aes(x=Site_cat, y = prop_diff), data = choice, alpha = 0.75, color = "grey20") +
...
ggplot() +
  ...
  geom_point(aes(x=Site_cat, y = abs_diff), alpha = 0.75, color = "grey20") +
  geom_point(aes(x = Site_cat, y = mean), size = 1.5, data = choice3, position = position_nudge(0.25)) +
  geom_errorbar(aes(x = Site_cat, y = mean, ymin = mean - se, ymax = mean + se), width = 0.15, data = choice3, position = position_nudge(0.25)) +
...
# Spider Position Across Days


```{r tunnel, echo = FALSE, message = FALSE, warning = FALSE}
tunnel_8B <- tunnel %>% 
  filter(Site == "8B") %>% 
  mutate(Dim.1 = c(scale(Dim.1)))
t1 <- glm(tunnel ~ day * Dim.1, data = tunnel_8B, family = binomial)
t2 <- step(t1)
Anova(t2) # day
...
t1 <- glm(tunnel ~ day * Dim.1, data = tunnel_8A, family = binomial)
t2 <- step(t1)
#Anova(t2)
...
t1 <- glm(tunnel ~ day * Dim.1, data = tunnel_6C, family = binomial)
t2 <- step(t1)
Anova(t2) # day
...
t1 <- glm(tunnel ~ day * Dim.1, data = tunnel_5A, family = binomial)
t2 <- step(t1)
#Anova(t2)
```
...
```{r}
loud <- choice_spider %>% 
  mutate(loud = ifelse(side_lq == "loud", 1, 0))
loud_scaled <- loud %>% 
  mutate(Age = c(scale(Age)),
         #mean_leq = factor(mean_leq),
         mean_leq = c(scale(mean_leq)),
         condition = c(scale(condition)))

l1 <- glm(loud ~ mean_leq * day * Age + mean_leq * day * condition, data = loud_scaled, family = binomial)
l2 <- step(l1)
l2 <- glm(loud ~ mean_leq * Age, data = loud_scaled, family = binomial)
Anova(l2)
...
pred = expand.grid(mean_leq = seq(-1, 1.4, 0.02),
                  ...
                  #              median(stay_scaled$mean_leq),
                  #              max(stay_scaled$mean_leq)),
                 Age = quantile(loud_scaled$Age, probs = c(0.25, 0.75)))
pred <- add_column(pred, fit = data.frame(predict(l2, newdata = pred, type = "response", se.fit = TRUE))$fit,
                  se = data.frame(predict(l2, newdata = pred, type = "response", se.fit = TRUE))$se.fit )
# back transform
pred$Age = pred$Age * sd(loud$Age) + mean(loud$Age)
pred$mean_leq = pred$mean_leq * sd(loud$mean_leq) + mean(loud$mean_leq)
...
quiet <- choice_spider %>% 
  mutate(quiet = ifelse(side_lq == "quiet", 1, 0))
quiet_scaled <- quiet %>% 
  mutate(Age = c(scale(Age)),
         #mean_leq = factor(mean_leq),
         mean_leq = c(scale(mean_leq)),
...
q1 <- glm(quiet ~ mean_leq * day * Age + mean_leq * day * condition, data = quiet_scaled, family = binomial)
q2 <- step(q1)
Anova(q2)
...
pred = expand.grid(mean_leq = seq(-1, 1.4, 0.02),
                  ...
                  #              median(stay_scaled$mean_leq),
                  #              max(stay_scaled$mean_leq)),
                 Age = quantile(quiet_scaled$Age, probs = c(0.25, 0.75)))
pred <- add_column(pred, fit = data.frame(predict(q2, newdata = pred, type = "response", se.fit = TRUE))$fit,
                  se = data.frame(predict(q2, newdata = pred, type = "response", se.fit = TRUE))$se.fit )
# back transform
pred$Age = pred$Age * sd(quiet$Age) + mean(quiet$Age)
pred$mean_leq = pred$mean_leq * sd(quiet$mean_leq) + mean(quiet$mean_leq)
...
There is a significant difference between 5A and 6C, but no difference with leq.
...
# site by side_pot
ggplot() +
  geom_bar(aes(x = Site, y = n, fill = side_lq), position="fill", stat="identity", data = sum2)

sum3 <- choice_spider %>% 
  group_by(day, side_lq) %>% 
  count() %>% 
  mutate(side_lq = factor(side_lq), 
         side_lq = fct_relevel(side_lq, "tunnel", "quiet", "loud"))
...
# day by side_pot
ggplot() +
  geom_bar(aes(x = day, y = n, fill = side_lq), position="fill", stat="identity", data = sum3)

sum_d4 <- choice_spider %>% 
  filter(day == 4) %>% 
  group_by(Site, Origin, side_lq) %>% 
  count() %>% 
  mutate(side_lq = factor(side_lq), 
...
day4 <- ggplot() +
  geom_bar(aes(x = Site, y = n, fill = side_lq), position="fill", stat="identity", data = sum_d4) + 
  geom_hline(yintercept = 0.5, linetype = 2)

sum_d1 <- choice_spider %>% 
  filter(day == 1) %>% 
  group_by(Site, Origin, side_lq) %>% 
  count() %>% 
  mutate(side_lq = factor(side_lq), 
...
day1 <- ggplot() +
  geom_bar(aes(x = Site, y = n, fill = side_lq), position="fill", stat="identity", data = sum_d1) + 
  geom_hline(yintercept = 0.5, linetype = 2)

ggarrange(day1, day4, nrow = 1, common.legend = TRUE)
```

```{r}
# make a data set that for each spider each day has 1 for the spider was in that part (tunnel, loud, or quiet) or 0 for they were not in that part
choice_spider2 <- choice_spider %>% 
  mutate(loud = ifelse(side_lq == "loud", 1, 0),
         quiet = ifelse(side_lq == "quiet", 1, 0),
         tunnel = ifelse(side_lq == "tunnel", 1, 0)) %>% 
  pivot_longer(loud:tunnel, values_to = "chosen", names_to = "side_pot")

choice_spider2_scaled <- choice_spider2 %>% 
  mutate(day = c(scale(day)),
         Age = c(scale(Age)),
         mean_leq = factor(mean_leq),
         #mean_leq = c(scale(mean_leq)),
         condition = c(scale(condition)))

mod <- glm(chosen ~ side_pot * Site, family = binomial, data = choice_spider2_scaled)
mod <- step(mod)
Anova(mod)
mod <- glm(chosen ~ 
             side_pot:day +
             ...
             side_pot:Age +
             side_pot:condition +
             side_pot:day:mean_leq + #removed
             side_pot:day:Age + #removed
             side_pot:day:condition + #removed
             side_pot:mean_leq:Age +
             side_pot:mean_leq:condition, family = binomial, data = choice_spider2_scaled)
mod <- step(mod)
Anova(mod)
lstrends(mod, pairwise ~ side_pot, var = "day")
lstrends(mod, pairwise ~ side_pot | mean_leq, var = "Age")
lstrends(mod, pairwise ~ mean_leq | side_pot, var = "Age")
lstrends(mod, pairwise ~ side_pot | mean_leq, var = "condition")
lstrends(mod, pairwise ~ mean_leq | side_pot, var = "condition")
```
