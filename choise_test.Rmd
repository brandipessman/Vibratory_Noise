...
library(flextable) # build tables
...
tunnel3 <- ggplot() +
  ...
  ylab("Proportion of Observations of \nSpiders in the Tunnel") +
  scale_fill_manual("Site Leq", 
                     values = c("#1b9e77", "#7570b3", "#d95f02"),
                     labels = c("-69", "-64", "-55")) +
  scale_color_manual("Site Leq", 
...
choice_lq_rural <- choice_lq %>% 
  filter(Site == "6C" | Site == "5A")
lq_rural <- glmer(side ~ Age + (1|ID), data = choice_lq_rural, family = binomial)
summary(lq_rural) # no diff
...
choice_part_scaled$ID <- factor(choice_part_scaled$ID)
...
```
```{r beta by site, echo = FALSE, warning = FALSE, message = FALSE}
...
#pred$part <- factor(pred$part, levels = c("loud", "quiet", "tunnel"),
#                  labels = c("Loud", "Quiet", "Tunnel"))

#prop_silk <- ggplot() +
#  geom_point(aes(x = Age, y = prop_part, group = factor(mean_leq), color = factor(mean_leq)), data = choice_part) +
#  geom_line(aes(x = Age, y = fit, group = factor(mean_leq), color = factor(mean_leq)), data = pred) +
#  geom_ribbon(aes(x = Age, y = fit, ymin = fit - se, ymax = fit + se, group = factor(mean_leq), fill = factor(mean_leq)), alpha = 0.5, data = pred) +
#  geom_hline(yintercept = 0.5, linetype = 2) +
#  scale_fill_manual("Site Leq", 
#                    values = c("#1B9E77", "#7570b3", "#D95F02"),
#                    labels = c("-69", "-64", "-55")) +
...
#jpeg("figures/prop_silk.jpeg", width = 6.5, height = 4, units = "in", quality = 100, res = 300)
#print(prop_silk)
#dev.off()
#prop_silk
...
#pred$part <- factor(pred$part, levels = c("loud", "quiet", "tunnel"),
#                  labels = c("Loud", "Quiet", "Tunnel"))
new_labels <- c("-69" = "Site Leq: -69 dB", "-64" = "Site Leq: -64 dB", "-55" = "Site Leq: -55 dB")

prop_silk <- ggplot() +
  geom_point(aes(x = Age, y = prop_part, group = part, color = part), data = choice_part) +
  geom_line(aes(x = Age, y = fit, group = part, color = part), data = pred) +
  geom_ribbon(aes(x = Age, y = fit, ymin = fit - se, ymax = fit + se, group = part, fill = part), alpha = 0.5, data = pred) +
  geom_hline(yintercept = 0.5, linetype = 2) +
  scale_fill_manual("Chamber Part", 
                    values = c("#e7298a", "#66a61e", "#e6ab02"),
                    labels = c("Loud", "Quiet", "Tunnel")) +
  scale_color_manual("Chamber Part", 
                    values = c("#e7298a", "#66a61e", "#e6ab02"),
                    labels = c("Loud", "Quiet", "Tunnel")) +
  scale_y_continuous(limits = c(0, 1), breaks = c(0, 0.25, 0.5, 0.75, 1)) +
  xlab("Age (Days Since Mature)") +
...
sides <- ggarrange(spider_side, silk_side, ncol = 2, common.legend = TRUE)
final <- ggarrange(sides, prop_silk, nrow = 2) 
jpeg("figures/final.jpeg", width = 6.5, height = 6.5, units = "in", quality = 100, res = 300)
print(final)
dev.off()
final
```
...
pred <- expand.grid(mean_leq = c(-0.8889, -0.1046, 1.3072))
pred <- add_column(pred, fit = predict(model.beta.gam.diff, newdata = pred, type = "response"),
                 se = predict(model.beta.gam.diff, newdata = pred, type = "response", se.fit = TRUE)$se.fit)
...
diff_leq <- ggplot() +
  geom_point(aes(x = mean_leq, y = prop_diff), data = choice, alpha = 0.75, color = "grey20") +
  geom_point(aes(x = mean_leq, y = fit), data = pred, size = 1.5, position = position_nudge(0.25)) +
  geom_errorbar(aes(x = mean_leq, ymin = fit - (se * 1.), ymax = fit + (se * 1.)), data = pred, width = 0.15, position = position_nudge(0.25)) +
  scale_y_continuous(limits = c(0, 1), breaks = c(0, 0.2, 0.5, 0.8, 1), labels = c("50:50", "60:40", "75:25", "90:10", "100:0")) +
  xlab("Site Leq") +
  ylab("Proportional Difference Between Sides") +
  th 
...
choice_stats <- choice_stats %>% 
  mutate(Focal = factor(Focal),
         ...
         Response = fct_recode(Response, "(i) Stay (1) or Move (0)" = "stay",
                               "(ii) Loud (1) or Quiet (0)" = "side",
                               "(iii) Loud (1) or Quiet (0)" = "silk_side",
                               "(iv) Total Silk Mass" = "total",
                               "(v) Proportion of Silk Mass" = "prop"), 
         Predictor = factor(Variable),
         Predictor = fct_recode(Predictor, "Leq" = "leq",
                               "Body Condition" = "cond",
                               "Age" = "age",
                               "Chamber Part" = "part",
                               "Leq x Body Condition" = "leqxcond",
                               "Leq x Age" = "leqxage",
                               "Chamber Part x Leq" = "partxleq",
                               "Chamber Part x Age" = "partxage",
                               "Chamber Part x Leq x Age" = "partxleqxage"))
...
flextable(choice_stats[, 1:8]) %>% 
  autofit() %>% 
  merge_v(j = c(' ', 'Distribution', 'Response', 
                'Test Statistic', 'Residual DF', 
                'R²'), part = "body") %>% 
  theme_vanilla() %>% 
  hline(i = c("1", "2", "4", "5", "7", "8", "10", "12", "13", "14", "15", "16"), 
  ...
  footnote(i = c(4), j = 8, ref_symbols = "b", value = as_paragraph("This test was a mixed model with spider ID as a random effect. As such, the R² value is the conditional R² The marginal R² value was 0.055.")) %>% 
  save_as_image(path = here::here("figures/choice_stats_table.png"))
