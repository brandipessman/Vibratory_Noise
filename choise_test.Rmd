...
library(insight) # R squared values for lmer and rlmer
...
summary(lq)
var.fix <- get_variance_fixed(lq)
var.ran <- get_variance_random(lq)
var.res <- get_variance_residual(lq)
R2m.lq = var.fix / (var.fix + var.ran + var.res)
R2c.lq = (var.fix + var.ran) / (var.fix + var.ran + var.res)
...
jpeg("figures/final.jpeg", width = 6.5, height = 7, units = "in", quality = 100, res = 300)
print(final)
dev.off()
final
```
...
flextable(choice_stats[, 1:8]) %>% 
  ...
  save_as_image(path = here::here("figures/choice_stats_table2.png"))
