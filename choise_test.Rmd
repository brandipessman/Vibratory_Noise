...
tunnel <- choice_spider %>% 
  ...
  #group_by(ID, Age, condition, Site, mean_leq, day) 
...
#drop1(tunnel.mod2, test = "Chisq")
#tunnel.mod3 <- update(tunnel.mod2, .~. -poly(mean_leq, 2):poly(condition, degree = 2))
#drop1(tunnel.mod3, test = "Chisq")
#tunnel.mod4 <- update(tunnel.mod3, .~. -poly(mean_leq, 2):day:poly(Age, degree = 2))
#drop1(tunnel.mod4, test = "Chisq")
#tunnel.mod5 <- update(tunnel.mod4, .~. -poly(mean_leq, 2):day)
#drop1(tunnel.mod5, test = "Chisq")
#tunnel.mod6 <- update(tunnel.mod5, .~. -day:poly(condition, degree = 2))
#drop1(tunnel.mod6, test = "Chisq")
#tunnel.mod7 <- update(tunnel.mod6, .~. -poly(condition, degree = 2))
#drop1(tunnel.mod7, test = "Chisq")
#tunnel.mod8 <- update(tunnel.mod7, .~. -day:poly(Age, degree = 2))
#drop1(tunnel.mod8, test = "Chisq")
#tunnel.mod9 <- update(tunnel.mod8, .~. -day)
...
tunnel.mod3 <- glmer(tunnel ~ mean_leq * poly(Age, 2) + (1|ID), data = tunnel_scaled, family = binomial) # no sig dif, winner winner chicken dinner
#anova(tunnel.mod2, tunnel.mod3)
summary(tunnel.mod3)
...
#tunnel.mod5 <- glm(tunnel ~ mean_leq * poly(Age, 2) + (mean_leq:poly(Age, 2)|ID), data = tunnel_scaled, family = binomial)
#anova(tunnel.mod3, tunnel.mod5) # better but not significantly
#1 is significantly better than 2 and 4, but not 3. 
...
```{r tunnel ever stats and assumptions, echo = FALSE, message = FALSE, warning = FALSE}
# was the spider ever found in the tunnel
tunnel2 <- choice_spider %>% 
  mutate(tunnel = ifelse(side_lq == "tunnel", 1, 0)) %>% 
  group_by(ID, Age, condition, Site, mean_leq) %>% 
  summarize(days_tunnel = sum(tunnel)) %>% 
  mutate(tunnel_any = ifelse(days_tunnel > 0, 1, 0))
tunnel_scaled2 <- tunnel %>% 
  ungroup() %>% 
  mutate(Age = c(scale(Age)),
         condition = c(scale(condition)),
         mean_leq = c(scale(mean_leq)))

# AIC model selection before
# all poly
#tunnel.mod <- glm(tunnel_any ~ poly(mean_leq, 2) * poly(Age, 2) + poly(mean_leq, 2) * poly(condition, 2), data = tunnel_scaled2, family = binomial) # 87.76
#summary(tunnel.mod)
#no poly
#tunnel.mod2 <- glm(tunnel_any ~ mean_leq * Age + mean_leq * condition, data = tunnel_scaled2, family = binomial)
#summary(tunnel.mod2) #86.11
#anova(tunnel.mod, tunnel.mod2, test = "Chisq")
#only age
#tunnel.mod3 <- glm(tunnel_any ~ mean_leq * poly(Age, 2) + mean_leq * condition, data = tunnel_scaled2, family = binomial) # 85
#summary(tunnel.mod3)
#anova(tunnel.mod2, tunnel.mod3, test = "Chisq") # best so far
#leq and age
tunnel.mod4 <- glm(tunnel_any ~ poly(mean_leq, 2) * poly(Age, 2) + poly(mean_leq, 2) * condition, data = tunnel_scaled2, family = binomial) # 82.99
summary(tunnel.mod4)
#anova(tunnel.mod3, tunnel.mod4, test = "Chisq") # this one is best
# age and condition
#tunnel.mod5 <- glm(tunnel_any ~ mean_leq * poly(Age, 2) + mean_leq * poly(condition, 2), data = tunnel_scaled2, family = binomial) # 87.52
#summary(tunnel.mod5)
#anova(tunnel.mod4, tunnel.mod5, test = "Chisq") # not this one
tunnel.mod <- glm(tunnel_any ~ poly(mean_leq, 2) * poly(Age, 2) + poly(mean_leq, 2) * condition, data = tunnel_scaled2, family = binomial) # 82.99
tunnel.mod <- step(tunnel.mod) # leq x condition + age
summary(tunnel.mod) 
...
tunnel.mod <- glm(tunnel_any ~ poly(mean_leq, 2) * poly(Age, 2), data = tunnel_scaled, family = binomial) # 77.38
summary(tunnel.mod)

#tunnel.mod2 <- glm(tunnel_any ~ mean_leq * Age, data = tunnel_scaled, family = binomial)
#summary(tunnel.mod2) # 82.6
#anova(tunnel.mod, tunnel.mod2, test = "Chisq") # sig, 1 better

#tunnel.mod3 <- glm(tunnel_any ~ mean_leq * poly(Age, 2), data = tunnel_scaled, family = binomial)
#summary(tunnel.mod3) # 81.09
#anova(tunnel.mod, tunnel.mod3, test = "Chisq") # better than 2 but not 1
with(summary(tunnel.mod), 1 - deviance/null.deviance)
...
tunnel.mod.8b <- glm(tunnel_any ~ Age, data = tunnel_8B, family = binomial)
summary(tunnel.mod.8b) # trend

tunnel_8A <- tunnel %>% 
  filter(Site == "8A")

tunnel.mod.8a <- glm(tunnel_any ~ Age, data = tunnel_8A, family = binomial)
summary(tunnel.mod.8a) # not enough data

tunnel_6C <- tunnel %>% 
  filter(Site == "6C")

tunnel.mod.6c <- glm(tunnel_any ~ Age, data = tunnel_6C, family = binomial)
summary(tunnel.mod.6c) # no diff

tunnel_5A <- tunnel %>% 
  filter(Site == "5A")

tunnel.mod.5a <- glm(tunnel_any ~ Age, data = tunnel_5A, family = binomial)
...
pred$fit <- predict(tunnel.mod3, newdata = pred, se.fit = TRUE, re.form = NA, type = "response")

#pred <- data.frame(Age = tunnel_scaled$Age, 
#                   mean_leq = tunnel_scaled$mean_leq,
#                   ID = tunnel_scaled$ID,
#                   fit = predict(tunnel.mod, newdata = tunnel_scaled, se.fit = TRUE, re.form = #NA, type = "response"))

myFunc <- function(mm) {
    predict(mm, newdata = pred, re.form = ~0, type = "response")
}
#bigBoot_tunnel3 <- bootMer(tunnel.mod3, myFunc, nsim = 1000)
#saveRDS(bigBoot_tunnel3, file = "data/bigBoot_tunnel3.Rds")
bigBoot_tunnel3 <- readRDS("data/bigBoot_tunnel3.Rds")
predSE <- t(apply(bigBoot_tunnel3$t, MARGIN = 2, FUN = sd))
pred$se <- predSE[1, ]
...
```{r tunnel ever graphs for leq, echo = FALSE, message = FALSE, warning = FALSE, results = 'hide'}
# did the spider ever go in the tunnel
pred <- expand.grid(mean_leq = seq(-1, 1.4, 0.02),
                  Age = quantile(tunnel_scaled$Age, probs = c(seq(0, 1, 0.1))))
pred <- add_column(pred, fit = data.frame(predict(tunnel.mod, newdata = pred, type = "response", se.fit = TRUE))$fit,
                  se = data.frame(predict(tunnel.mod, newdata = pred, type = "response", se.fit = TRUE))$se.fit )
#pred <- data.frame(effect("mean_leq*Age", tunnel.mod, se.fit = TRUE, residuals = TRUE, seq(-2.5, 2.5)))
...
pred2 <- expand.grid(mean_leq = seq(-1, 1.4, 0.02),
                  Age = quantile(tunnel_scaled$Age, probs = c(0, 0.5, 1)))
pred2 <- add_column(pred2, fit = data.frame(predict(tunnel.mod, newdata = pred2, type = "response", se.fit = TRUE))$fit,
                  se = data.frame(predict(tunnel.mod, newdata = pred2, type = "response", se.fit = TRUE))$se.fit )

# back transform
pred2$mean_leq = pred2$mean_leq * sd(tunnel$mean_leq) + mean(tunnel$mean_leq)
pred2$Age = pred2$Age * sd(tunnel$Age) + mean(tunnel$Age)

tunnel2 <- ggplot() +
  geom_ribbon(aes(x = mean_leq, y = fit, ymin = fit - (1 * se), ymax = fit + (1 * se), group = Age, color = Age, fill = Age), data = pred2, alpha = 0.65) +
  geom_line(aes(x = mean_leq, y = fit, group = Age, color = Age), data = pred2, linewidth = 1) +
  geom_line(aes(x = mean_leq, y = fit, group = Age, color = Age), data = pred, linewidth = 0.5) +
  geom_jitter(aes(x = mean_leq, y = tunnel_any, group = Age, color = Age), data = tunnel, width = 1.5, height = 0.05, alpha = 1, size = 1) +
  geom_hline(yintercept = 0.5, linetype = 2) +
  xlab("Site Average Leq (dB, 20-1000 Hz)") +
  ylab("Proportion of Spiders that \nwere Found in the Tunnel") +
  scale_x_continuous(breaks = c(-69, -64, -55)) +
  scale_fill_viridis_c(limits = c(0, 50), breaks = c(0, 25, 50), name = "Age (Days Mature)", option = "plasma", direction = -1) +
  scale_color_viridis_c(limits = c(0, 50), breaks = c(0, 25, 50), name = "Age (Days Mature)", option = "plasma", direction = -1) +
  th +
  theme(legend.position = "none")

jpeg("figures/tunnel.jpeg", width = 3.5, height = 2.5, units = "in", quality = 100, res = 300)
print(tunnel2)
dev.off()
tunnel2
```
...
summary(lq)
with(summary(lq), 1 - deviance/null.deviance)
...
pred <- expand.grid(mean_leq = seq(-1, 1.4, 0.02),
                  Age = quantile(choice_lq_scaled$Age, probs = c(0.25, 0.75)))
...
pred <- pred %>% 
  mutate(Age = factor(round(Age)),
         Age = fct_recode(Age, "younger" = "20", "older" = "32"),
         Age = fct_relevel(Age, "younger", "older"))

spider_side <- ggplot() +
    geom_jitter(aes(x = mean_leq, y = side, color = Age), data = data, width = 1, height = 0.05, alpha = 0.5) +
...
pred <- expand.grid(mean_leq = quantile(choice_lq_scaled$mean_leq, probs = c(0.25, 0.5, 0.75)),
                  Age = seq(-2, 2.5, 0.05))
pred$fit <- predict(lq9, newdata = pred, se.fit = TRUE, re.form = NA, type = "response")

myFunc <- function(mm) {
    predict(mm, newdata = pred, re.form = ~0, type = "response")
}
#bigBoot_spider_side2 <- bootMer(lq9, myFunc, nsim = 1000)
#saveRDS(bigBoot_spider_side2, file = "data/bigBoot_spider_side2.Rds")
bigBoot_spider_side2 <- readRDS("data/bigBoot_spider_side2.Rds")
predSE <- t(apply(bigBoot_spider_side2$t, MARGIN = 2, FUN = sd))
pred$se <- predSE[1, ]
...
pred$mean_leq = pred$mean_leq * sd(choice_lq$mean_leq) + mean(choice_lq$mean_leq)
pred <- pred %>% 
  mutate(mean_leq = factor(round(mean_leq)))

ggplot() +
    geom_jitter(aes(x = Age, y = side, color = factor(mean_leq)), data = choice_lq, width = 1, height = 0.05, alpha = 0.5) +
