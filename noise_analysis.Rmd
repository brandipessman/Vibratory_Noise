...
#saveRDS(combined_dfl, file = "/Users/bjpessman/Documents/phd_research_code/Vibratory_Noise/data/combined_dfl.rds")
combined_dfl <- readRDS("/Users/bjpessman/Documents/phd_research_code/Vibratory_Noise/data/combined_dfl.rds")
...
```{r site_map, echo = FALSE, warning = FALSE, message = FALSE}
# grabs the coordinates of Lincoln's city limits
map_data <- data.frame(read.table("/Users/bjpessman/Documents/phd_research_code/Vibratory_Noise/data/lincoln_coordinates.txt", sep = "," , header = F)) %>% 
  slice(10:2350) %>% 
  separate(V1, c("extra", "longitude", "latitude"), " ") %>% 
  mutate(longitude = as.numeric(longitude),
         latitude = as.numeric(latitude))

map_data2 <- data.frame(read.table("/Users/bjpessman/Documents/phd_research_code/Vibratory_Noise/data/wilderness_coordinates.txt", sep = "," , header = F)) %>% 
...
nebraska <- subset(states, region == "nebraska")
counties <- map_data("county")
ne_county <- subset(counties, region == "nebraska")
lancaster <- subset(ne_county, subregion == "lancaster")

# adjust main map view
height <- max(lancaster$lat) - min(lancaster$lat)
width <- max(lancaster$long) - min(lancaster$long)
...
# final plot
site_map <- ggmap(map) + 
  geom_polygon(data = map_data, aes(x = longitude, y = latitude), fill = "slategray", alpha = 0.25, color = "gray30") +
  geom_polygon(data = map_data2, aes(x = longitude, y = latitude), fill = "#7570b3", color = "gray30") +
  geom_point(data = sites, mapping = aes(x = Longitude, y = Latitude, group = Category, color = Category), pch=21, size = 1, fill = "black", stroke = 2) +
  geom_label_repel(aes(x =Longitude, y = Latitude, label = Site, group = Category, color = Category, fill = choice), min.segment.length = 0.1, max.overlaps = Inf, size = 3, data = sites, show.legend  = FALSE) +
...
jpeg("figures/sites.jpeg", width = 4.5, height = 6, units = "in", quality = 100, res = 300)
print(site_map)
dev.off()
...
```{r leq_map, echo = FALSE, warning = FALSE, message = FALSE}
leq_map <- ggmap(map) + 
  theme_classic() + 
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1)) +
  geom_polygon(data = map_data, aes(x = longitude, y = latitude), fill = "slategray", alpha = 0.25, color = "gray30") +
  geom_point(data = sites, mapping = aes(x = Longitude, y = Latitude), pch=21, size = 1, fill = "black") +
  #geom_density2d(data = sites, aes(x = Longitude, y = Latitude), size = 0.01, bins = 50) +
  #stat_density2d(data = sites, aes(x = Longitude, y = Latitude, fill = ..level.., alpha = ..level..), geom = "polygon", size = 0.01, bins = 50) +
  ...
  scale_fill_viridis(name = "Leq", option = "C") + # #1B9E77, #D95F02
  ylab("Latitude") + xlab("Longitude") +
  labs(fill = "Avg. Leq (dB)") +
...
```{r spatial, echo = FALSE, warning = FALSE, message = FALSE}
# First I upload a file that includes the date that each recording started on.
...
#normal 
dayavgl %>% 
  ggplot(aes(x = mean_leq, color = Substrate)) +
  geom_density()
qqp(dayavgl$mean_leq, "norm") 

# lognormal
qqp(abs(dayavgl$mean_leq), "lnorm") 
...
nbinom <- fitdistr(int_abs$mean_leq, densfun = "Negative Binomial", lower=c(1e-9, 1e-9))
qqp(int_abs$mean_leq, "nbinom", size = nbinom$estimate[[1]], mu = nbinom$estimate[[2]]) 

# poisson 
poisson <- fitdistr(int_abs$mean_leq, "Poisson")
qqp(int_abs$mean_leq, "pois", poisson$estimate, lambda = poisson$sd)

# gamma
gamma <- fitdistr(int_abs$mean_leq, "gamma")
qqp(int_abs$mean_leq, "gamma", shape = gamma$estimate[[1]], rate = gamma$estimate[[2]])

#dayavgl.lmer <- lmer(mean_leq ~ Dim.1 + Substrate + (1|Site), data = dayavgl)
#car::Anova(dayavgl.lmer)
#summary(dayavgl.lmer)
dayavgl.lm <- lm(mean_leq ~ Dim.1 * Substrate, data = dayavgl)
summary(dayavgl.lm)

test_dayavgl <- augment(dayavgl.lm, data = dayavgl)
...
scaleloc_dayavgl <- ggplot(test_dayavgl, aes(x = .fitted, y = sqrt(abs(.std.resid)))) + 
  geom_point() + 
  geom_smooth() + 
...
lev_dayavgl <- ggplot(test_dayavgl, aes(.hat, .std.resid)) +
...
ggarrange(resid_dayavgl, qq_dayavgl, scaleloc_dayavgl, lev_dayavgl, 
          labels = c("A", "B", "C", "D"),
          ncol = 2, nrow = 2)
...
# let's get a data frame with just 2020 data
dayavgl_20 <- dayavgl %>% 
...
space <- ggplot() +
  ...
  geom_line(aes(x = Dim.1, y = .fitted, color = Substrate), data = predictions) +
  geom_ribbon(aes(x = Dim.1, ymax = .fitted + .se.fit, ymin = .fitted - .se.fit, fill = Substrate, color = Substrate), data = predictions, alpha = 0.5) +
...
raw_hourly2 <- houravg %>% 
  ...
  scale_fill_manual("Category", values = c("#D95F02", "#1B9E77"),
                     labels = c("Urban", "Rural")) +
  scale_x_continuous(limits = c(0, 23.5), breaks = c(0, 5, 7, 8, 9, 10, 15, 19, 20, 23)) +
  scale_y_continuous(limits = c(-75, -42), breaks = c(-75, -70, -65, -60, -55, -50, -45)) +
  theme_classic() +
  theme(text = element_text(size = 10, color = "black"),
        legend.position = "top")

jpeg("figures/raw_hourly2.jpeg", 3.25, height = 4, units = "in", quality = 100, res = 300)
print(raw_hourly2)
dev.off()
...
```{r vibemap, echo = FALSE, warning = FALSE, message = FALSE}
siteavg <- combined_dfl %>% 
  group_by(Site) %>% 
  summarize(mean_leq = mean(Leq))
...
vibe <- ggplot(vibmap, aes(x = Visit, y = dark_light, fill = meanleq)) +
  geom_tile(color = "white", size = 0.1, na.rm = TRUE) + 
  scale_fill_viridis(name = "Hourly Leq", option = "C") + 
  facet_grid(Sub_Mic ~ Site) +
  scale_y_continuous(trans = "reverse", breaks = unique(vibmap$dark_light)) +
  theme_minimal(base_size = 8) +
  labs(x = "Visit Number (1-4)", y = "Hour Since Sunrise") +
...
```{r land_cover, echo = FALSE, warning = FALSE, message = FALSE}
land_cover <- read.csv("data/site_land_cover.csv", header = TRUE) %>% 
  mutate(Site = factor(Site),
         Site = fct_relevel(Site, "7B", "6B", "6C", "7A", "5A", "6A", "3B", "1A", "2C", "5C", "7C", "4A", "4C", "2B", "5B", "3C", "1B", "8B", "2A", "1C", "3A", "8A", "4B"),
