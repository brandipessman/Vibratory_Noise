...
space2 <- ggplot() +
  geom_point(aes(x = Dim.2, y = mean_leq, color = Substrate), data = dayavgl) +
...
coefs.robustr.s$P <- round(2*pt(abs(coefs.robustr.s[,3]), season.lmer.stats$devcomp$dims[4], lower=FALSE), 10)
...
season <- ggplot() +
  ...
  geom_line(aes(x = as.numeric(Visit) + 0.25, y = response, color = Category, group = Category), data = predictions, linewidth = 1) +
  geom_errorbar(aes(x = as.numeric(Visit) + 0.25, ymax = response + SE, ymin = response - SE, color = Category, group = Category), data = predictions, width = 0.25, linewidth = 1) +
...
season_robust <- ggplot() +
  geom_point(aes(x = factor(Visit), y = mean_leq, color = Category), data = dayavgl) +
  geom_point(aes(x = Visit2 + 0.25, y = fit, group = Category2, color = Category2), data = predictions, size = 2) +
  geom_line(aes(x = Visit2 + 0.25, y = fit, color = Category2), data = predictions) +
  geom_errorbar(aes(x = Visit2 + 0.25, ymax = fit + se, ymin = fit - se, color = Category2), data = predictions, width = 0.25) +
  geom_line(data = group2v3, aes(x= x, y = y, group=1), inherit.aes = F) +
  xlab("Visit") +
  ylab("Daily Average Leq (dB, 20-1000 Hz)") +
  scale_color_manual("Category", values = c("#1B9E77", "#D95F02"),
